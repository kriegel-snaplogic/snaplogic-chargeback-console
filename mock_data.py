...
# Optional live backend: a SnapLogic triggered task that returns runtime events
# from Snowflake. Both must be set for fetch_exec_from_snowflake() to do anything;
# with neither set the app runs on the bundled demo fixtures. Configure in .env —
# see .env.example. Never hardcode the token here.
_SNOWFLAKE_TASK_URL = os.getenv("SL_READ_ENDPOINT", "")
_SNOWFLAKE_BEARER   = os.getenv("SL_READ_BEARER", "")
...
def _snaplex_from_path(runtime_path_id, snaplexes):
    """Map RUNTIME_PATH_ID (e.g. 'DemoOrg/rt/cloud/dev') to a snaplex id."""
...
def _bu_from_path(pipeline_path, project_mappings):
    """Map a pipeline PATH like '/DemoOrg/Avery/PROD_MCP' to a BU id."""
...
def fetch_exec_from_snowflake():
    ...
    Returns None when SL_READ_ENDPOINT / SL_READ_BEARER are unset, which is the
    normal case for the bundled demo — callers already treat None as "no live
    data" and fall back to the fixtures."""
    if not (_SNOWFLAKE_TASK_URL and _SNOWFLAKE_BEARER):
        return None
    try:
...
def start_snowflake_refresh():
    """Kick off a background thread to fetch Snowflake data. No-op unless the
    live backend is configured, so the demo does not spawn a pointless thread."""
    if not (_SNOWFLAKE_TASK_URL and _SNOWFLAKE_BEARER):
        return
...
# Demo business units. Owners are role titles rather than names — the console is
# meant to be re-pointed at your own org, so edit these in BU Management.
DEFAULT_BUS = [
    {"id": "bu_genai",     "name": "GenAI & Integration Platform", "cost_center": "CC-1001",
     "owner": "GenAI Lead",       "headcount": 20},
    ...
    {"id": "bu_proserv",   "name": "Professional Services",        "cost_center": "CC-6001",
     "owner": "ProServ Lead",     "headcount": 5},
    {"id": "bu_other",     "name": "Other / External",             "cost_center": "CC-9999",
     "owner": "Various",          "headcount": 4},
]

DEFAULT_SNAPLEXES = [
    # Illustrative Snaplex topology for the demo. node_cost_monthly values are
    # placeholders — replace with your own contracted node cost in Cost
    # Configuration before reading anything into the numbers.
    ...
    {"id": "slx_eks",    "name": "EKS K8s EMEA Groundplex",     "type": "shared",     "bu_id": None,
     ...
     "snode_id": "DemoOrg/rt/sidekick/demo",
    ...
    {"id": "slx_linux",  "name": "EMEA Linux Groundplex (EC2)",  "type": "dedicated",  "bu_id": "bu_training",
     ...
     "snode_id": "DemoOrg/rt/sidekick/emea-linux-ec2",
...
DEFAULT_USER_MAPPINGS = [
    ...
    # GenAI
    {"username": "Avery",                "bu_id": "bu_genai"},
    {"username": "Juno",                 "bu_id": "bu_genai"},
    ...
    # Platform
    {"username": "Nolan",                "bu_id": "bu_platform"},
    {"username": "Aspen",                "bu_id": "bu_platform"},
    {"username": "Quinn",                "bu_id": "bu_platform"},
    {"username": "projects",             "bu_id": "bu_platform"},
    {"username": "autosync",             "bu_id": "bu_platform"},
    # Pre-Sales
    {"username": "Finley",               "bu_id": "bu_presales"},
    {"username": "Reese",                "bu_id": "bu_presales"},
    {"username": "Marlowe",              "bu_id": "bu_presales"},
    # Professional Services
    {"username": "Skyler Project Space", "bu_id": "bu_proserv"},
    # Insurance
    {"username": "Tatum",                "bu_id": "bu_insurance"},
    {"username": "Devon",                "bu_id": "bu_insurance"},
    {"username": "Ellis",                "bu_id": "bu_insurance"},
    {"username": "Briar",                "bu_id": "bu_insurance"},
    # Healthcare
    {"username": "Harper",               "bu_id": "bu_healthcare"},
    {"username": "Cameron",              "bu_id": "bu_healthcare"},
    {"username": "00_TeamHealthcareB",   "bu_id": "bu_healthcare"},
    {"username": "blake",                "bu_id": "bu_healthcare"},
    # MCP COE
    {"username": "apim",                 "bu_id": "bu_mcp"},
    # Other / External
    {"username": "PartnerThree",         "bu_id": "bu_other"},
]

DEFAULT_PROJECT_MAPPINGS = [
    # Demo project-space → BU mappings, matching the bundled user_state.json.
    # GenAI & Integration Platform
    {"project_path": "/DemoOrg/Avery",                    "bu_id": "bu_genai"},
    {"project_path": "/DemoOrg/Juno",                     "bu_id": "bu_genai"},
    {"project_path": "/DemoOrg/Agent Creator",            "bu_id": "bu_genai"},
    {"project_path": "/DemoOrg/snapLogic4snapLogic",      "bu_id": "bu_genai"},
    {"project_path": "/DemoOrg/toolsasapi",               "bu_id": "bu_genai"},
    {"project_path": "/DemoOrg/toolsasmcp",               "bu_id": "bu_genai"},
    # Platform Engineering
    {"project_path": "/DemoOrg/Nolan",                    "bu_id": "bu_platform"},
    {"project_path": "/DemoOrg/Aspen",                    "bu_id": "bu_platform"},
    {"project_path": "/DemoOrg/projects",                 "bu_id": "bu_platform"},
    {"project_path": "/DemoOrg/autosync",                 "bu_id": "bu_platform"},
    # Pre-Sales Engineering
    {"project_path": "/DemoOrg/Finley",                   "bu_id": "bu_presales"},
    {"project_path": "/DemoOrg/Reese",                    "bu_id": "bu_presales"},
    {"project_path": "/DemoOrg/Skyler Project Space",     "bu_id": "bu_proserv"},
    {"project_path": "/DemoOrg/Quinn",                    "bu_id": "bu_platform"},
    {"project_path": "/DemoOrg/Marlowe",                  "bu_id": "bu_presales"},
    # Insurance Practice
    {"project_path": "/DemoOrg/Tatum",                    "bu_id": "bu_insurance"},
    # Healthcare Practice
    {"project_path": "/DemoOrg/Harper",                   "bu_id": "bu_healthcare"},
    # MCP Platform COE
    {"project_path": "/DemoOrg/apim",                     "bu_id": "bu_mcp"},
    # Other / External
    # Insurance Practice
    {"project_path": "/DemoOrg/Devon",                    "bu_id": "bu_insurance"},
    {"project_path": "/DemoOrg/Ellis",                    "bu_id": "bu_insurance"},
    {"project_path": "/DemoOrg/Briar",                    "bu_id": "bu_insurance"},
    # Healthcare Practice
    {"project_path": "/DemoOrg/Cameron",                  "bu_id": "bu_healthcare"},
    {"project_path": "/DemoOrg/00_TeamHealthcareB",       "bu_id": "bu_healthcare"},
    {"project_path": "/DemoOrg/blake",                    "bu_id": "bu_healthcare"},
    # Other / External
    {"project_path": "/DemoOrg/PartnerThree",             "bu_id": "bu_other"},
]


def _generate_exec_data():
    rng = random.Random(42)
    # Base volumes are shaped to look like a real month of platform activity
    # (duration_sec uses state_timestamp - create_time, capped at 60 min per
    # execution), so the cost split across BUs is plausible rather than uniform:
    #
    # bu_genai:    MCP tool pipelines (slx_cloud: 77k × 1.7s) + agent pipelines
    #              (slx_eks: 7.3k × 128s avg, capped) → ~17.8k min
    # bu_presales: POC pipelines (slx_eks: 2.1k × 13s) + demo agents
    #              (slx_cloud: 80 × 260s) → ~814 min
...
def init_session_state(st):
    ...
    if "domain_rules" not in st.session_state:
        # Domain rules route everyone at a given external domain to one BU —
        # typically partners and contractors. These defaults point at the
        # placeholder domains in the bundled demo user list; replace them with
        # your own partner domains in Asset Mapping → Project Rules.
        st.session_state.domain_rules = [
            {"domain": "ext-591bfe.com", "bu_id": "bu_other"},
            {"domain": "ext-f34ab7.com", "bu_id": "bu_other"},
            {"domain": "ext-26da4f.com", "bu_id": "bu_other"},
            {"domain": "ext-387eea.com", "bu_id": "bu_other"},
            {"domain": "ext-7bf6fa.com", "bu_id": "bu_other"},
            {"domain": "ext-9c4f44.com", "bu_id": "bu_other"},
            {"domain": "ext-a876a8.com", "bu_id": "bu_other"},
            {"domain": "ext-84f9b1.com", "bu_id": "bu_other"},
            {"domain": "ext-1dfcd7.com", "bu_id": "bu_other"},
        ]
    ...
    # Auto-register DemoOrg connection from env vars on any page load
    if not st.session_state.environments and not st.session_state.get("_auto_connect_tried"):
        ...
        import os as _os
        _server = _os.getenv("SNAPLOGIC_SERVER", "https://elastic.snaplogic.com")
        _org    = _os.getenv("SNAPLOGIC_ORG", "DemoOrg")
        ...
        if _auth:
            try:
                ...
                _ok = False
            _entry = {
                "name": "DemoOrg", "server": _server, "org": _org,
